            ...
            for (int b = 0; b < path.Count(); b++)
            {
                ...
                */
                ...
                if (region == JP)
                {
                    ...
                    fsr_toc.Seek(1097704, SeekOrigin.Begin);
                    fsr_toc.Read(bs_toc, 0, bs_toc.Length);
                }
                if (region == US)
                {
                    bs_toc = new byte[18176];
                    fsr_toc.Seek(1102424, SeekOrigin.Begin);
                    fsr_toc.Read(bs_toc, 0, bs_toc.Length);
                }
                if (region == PAL)
                {
                    bs_toc = new byte[19328];
                    fsr_toc.Seek(1115064, SeekOrigin.Begin);
                    fsr_toc.Read(bs_toc, 0, bs_toc.Length);
                }
                fsr_toc.Close();

                int writefile_start_seek = 0;
                int writefile_length_seek = 8;
